...
public class Almanac {
	...
	public void changePop(String searchName, int newPopulation) {
		...
		for (int i = 0; i < this.list.length; i ++) {
			if (list[i].getName().equals(searchName)) {
				list[i] = new City(newPopulation, searchName); 
			}
		}
	...
	*/
	public void increasePop(double rate) { //check
				
		int increase = 0; 
		
		for (int i = 0; i < this.list.length; i ++) {
			increase = (int)((1 + (rate/100)) * list[i].getPopulation() + 0.5); 
			list[i] = new City(increase, list[i].getName());
		}
